...
func validateConfig(config *ConfigStruct) (err error) {
	...
	if config == nil {
		return log.Err("NOT VALID: No Config Struct")
	}

	if len(config.NodeName) == 0 {
		return log.Err("NOT VALID: No NodeName defined")
	}

	if len(config.Sites) == 0 {
		return log.Err("NOT VALID: No Sites defined")
	}

	for i := 0; i < len(config.Sites); i++ {
		if err = validateSite(&config.Sites[i]); err != nil {
			log.Errorf("Site Error[%d]%s", i, err)
			return err
		}
	}
...
func validateConnection(connection *ConnectionStruct) (err error) {
	...
	if len(connection.Host) == 0 {
		return log.Err("Host Name Required")
	}
...
func validateMtu(mtu *ConfigMtuType) (err error) {
	...
	if *mtu < ConfigMtuMin || *mtu > ConfigMtuMax {
		return log.Errf("MTU Size:%d outside the MIN/MAX boundries %d/%d", *mtu, ConfigMtuMin, ConfigMtuMax)
	}
...
func validatePort(port *ConfigPortType) (err error) {
	...
	if *port < ConfigPortMin || *port > ConfigPortMax {
		return log.Errf("MTU Size:%d outside the MIN/MAX boundries %d/%d", *port, ConfigPortMin, ConfigPortMax)
	}
...
func validateProto(proto *ConfigProtoType) (err error) {
	if string(*proto) == "" {
		return log.Errf("Protocol Required")
	}
	...
	case UDP6:
	default:
		return log.Errf("Unknown Protocol:%s", *proto)
	}
