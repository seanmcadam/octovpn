...
func ReadConfig(filepath string) (config *ConfigStruct, err error) {
	...
	if err != nil {
		return nil, loggy.Errf("READ ERROR: opening file: %s", err)
	}
	defer file.Close()
	...
	if err != nil {
		return nil, loggy.Errf("READ ERROR: reading file: %s", err)
	}
	...
	if err = json.Unmarshal(rawFileByteValue, &Config); err != nil {
		return nil, loggy.Errf("READ ERROR: marshaling JSON: %s", err)
	}
